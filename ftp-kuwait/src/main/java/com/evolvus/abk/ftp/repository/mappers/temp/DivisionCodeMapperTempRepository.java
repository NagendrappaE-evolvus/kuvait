...
public interface DivisionCodeMapperTempRepository extends CrudRepository<FTPDivisionCodeMapperTemp, Long> {
	...
	@Query(value ="SELECT tempMapper.gl_sub_head_code FROM FTP_FINAL_DIVISION_MAPPER_TEMP tempMapper where "
			...
			"mapper.final_division_desc) from FTP_FINAL_DIVISION_MAPPER mapper)",
			nativeQuery=true)
