...
public interface DivisionCodeMapperRepository extends CrudRepository<FTPDivisionCodeMapper, Long> {
	...
	@Query(value ="SELECT mapper.gl_sub_head_code FROM FTP_FINAL_DIVISION_MAPPER mapper where mapper.bank_code=:bankCode and "
			...
			"tempMapper.final_division_desc) from FTP_FINAL_DIVISION_MAPPER_TEMP tempMapper where tempMapper.bank_code=:bankCode)",
			nativeQuery=true)
