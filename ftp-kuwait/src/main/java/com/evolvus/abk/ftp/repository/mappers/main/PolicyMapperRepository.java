...
public interface PolicyMapperRepository extends CrudRepository<FTPPolicyMapper, Long> {
	...
	@Query(value = "SELECT mapper.ftp_category FROM ftp_curve_mapper mapper where mapper.bank_code=:bankCode and CONCAT(mapper.ftp_category,mapper.ccy_code_in,mapper.ccy_code_not_in,mapper.division_code_in,mapper.division_code_not_in,"
			...
			+ "mapper.subdivision_code_not_in,mapper.fixed_length,mapper.maturity_date,mapper.base_tenor,mapper.margin_tenor,mapper.applicable_curve,mapper.pre_post,mapper.final_ftp_category) not in (SELECT CONCAT(tempMapper.ftp_category,tempMapper.ccy_code_in,tempMapper.ccy_code_not_in,tempMapper.division_code_in,tempMapper.division_code_not_in,"
			...
			+ "tempMapper.subdivision_code_not_in,tempMapper.fixed_length,tempMapper.maturity_date,tempMapper.base_tenor,tempMapper.margin_tenor,tempMapper.applicable_curve,tempMapper.pre_post,tempMapper.final_ftp_category) FROM ftp_curve_mapper_temp tempMapper where tempMapper.bank_code=:bankCode)", nativeQuery = true)
