...
public interface PolicyMapperRepository extends CrudRepository<FTPPolicyMapper, Long> {
	...
	@Query(value = "SELECT mapper.ftp_category FROM ftp_curve_mapper mapper where CONCAT(mapper.ftp_category,mapper.ccy_code_in,mapper.ccy_code_not_in,mapper.division_code_in,mapper.division_code_not_in,"
			...
			+ "tempMapper.subdivision_code_not_in,tempMapper.fixed_length,tempMapper.maturity_date,tempMapper.base_tenor,tempMapper.margin_tenor,tempMapper.applicable_curve,tempMapper.pre_post,tempMapper.final_ftp_category) FROM ftp_curve_mapper_temp tempMapper)", nativeQuery = true)
