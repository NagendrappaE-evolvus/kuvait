...
public interface GrandMapperTempRepository extends CrudRepository<FTPGrandMapperTemp, Long> {
	...
	@Query(value = "SELECT tmpMapper.gl_subhead_code FROM ftp_grand_mapper_temp tmpMapper where tmpMapper.bank_id=:bankCode and "
			...
			+ "mapper.instrument_class_not_in,mapper.group_by_logic,mapper.count) from ftp_grand_mapper mapper where mapper.bank_id=:bankCode)", nativeQuery = true)
