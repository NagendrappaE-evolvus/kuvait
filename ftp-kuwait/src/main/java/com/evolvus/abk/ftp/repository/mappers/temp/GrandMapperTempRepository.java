...
public interface GrandMapperTempRepository extends CrudRepository<FTPGrandMapperTemp, Long> {
	...
	@Query(value = "SELECT tmpMapper.gl_sub_head_code FROM ftp_grand_mapper_temp tmpMapper where tmpMapper.bank_id=:bankCode and "
			+ "CONCAT(tmpMapper.gl_sub_head_code,tmpMapper.gl_sub_head_desc,tmpMapper.dr_ftp_category,tmpMapper.cr_ftp_category,tmpMapper.user_subclass_code_in,tmpMapper.user_subclass_code_not_in,"
			...
			+ "tmpMapper.instrument_sub_class_not_in,tmpMapper.group_by_logic,tmpMapper.count) not in (SELECT CONCAT(mapper.gl_sub_head_code,mapper.gl_sub_head_desc,mapper.dr_ftp_category,mapper.cr_ftp_category,mapper.user_subclass_code_in,mapper.user_subclass_code_not_in,"
