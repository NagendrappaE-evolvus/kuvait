...
public interface GrandMapperTempRepository extends CrudRepository<FTPGrandMapperTemp, Long> {
	...
	@Query(value ="SELECT tmpMapper.gl_subhead_code FROM ftp_grand_mapper_temp tmpMapper where "
			...
			"mapper.instrument_class_not_in,mapper.group_by_logic,mapper.count) from ftp_grand_mapper mapper)",
			nativeQuery=true)
	List<String> fetchRecordsNotInMain();
	
	List<FTPGrandMapperTemp> findByGlSubheadCodeInOrderByGlSubheadCode(Set<String> glSubheadCode);
