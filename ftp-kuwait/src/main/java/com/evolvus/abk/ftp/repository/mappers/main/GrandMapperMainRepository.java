...
public interface GrandMapperMainRepository extends CrudRepository<FTPGrandMapper, Long> {
	...
	@Query(value ="SELECT mapper.gl_subhead_code FROM ftp_grand_mapper mapper where mapper.bank_code=:bankCode and "
			...
			"tmpMapper.instrument_class_not_in,tmpMapper.group_by_logic,tmpMapper.count) from ftp_grand_mapper_temp tmpMapper where tmpMapper.bank_code=:bankCode)",
			nativeQuery=true)
