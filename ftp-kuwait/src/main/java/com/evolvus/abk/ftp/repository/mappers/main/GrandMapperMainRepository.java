...
public interface GrandMapperMainRepository extends CrudRepository<FTPGrandMapper, Long> {
	...
	@Query(value ="SELECT mapper.gl_subhead_code FROM ftp_grand_mapper mapper where "
			...
			"tmpMapper.instrument_class_not_in,tmpMapper.group_by_logic,tmpMapper.count) from ftp_grand_mapper_temp tmpMapper)",
			nativeQuery=true)
	List<String> fetchRecordsNotInTemp();
	
	List<FTPGrandMapper> findByGlSubheadCodeInOrderByGlSubheadCode(Set<String> glSubheadCode);
}
