...
public interface GrandMapperMainRepository extends CrudRepository<FTPGrandMapper, Long> {
	...
	@Query(value ="SELECT mapper.gl_sub_head_code FROM ftp_grand_mapper mapper where mapper.bank_id=:bankCode and "
			+ "CONCAT(mapper.gl_sub_head_code,mapper.gl_sub_head_desc,mapper.dr_ftp_category,mapper.cr_ftp_category,mapper.user_subclass_code_in,mapper.user_subclass_code_not_in," + 
			...
			"mapper.instrument_sub_class_not_in,mapper.group_by_logic,mapper.count) not in (SELECT CONCAT(tmpMapper.gl_sub_head_code,tmpMapper.gl_sub_head_desc,tmpMapper.dr_ftp_category,tmpMapper.cr_ftp_category,tmpMapper.user_subclass_code_in,tmpMapper.user_subclass_code_not_in," + 
