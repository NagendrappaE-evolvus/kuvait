...
public interface ProductMapperMainRepository extends CrudRepository<FTPProductMapper, Long>{
	...
	@Query(value ="SELECT mapper.ftp_category FROM ftp_product_mapper mapper where mapper.bank_id=:bankCode and "
			...
			"tempMapper.core_non_core,tempMapper.core_prnt) from ftp_product_mapper_temp tempMapper where tempMapper.bank_id=:bankCode)",
			nativeQuery=true)
	List<String> fetchRecordsNotInTemp(@Param("bankCode")String bankCode);

	List<FTPProductMapper> findByFtpCategoryInAndBankCodeOrderByFtpCategory(Set<String> ftpCategory,FtpEntity ftpEntity);
